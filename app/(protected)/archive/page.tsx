...
import {
  listArchiveForAdmin,
  listArchiveForAuthor,
  countArchiveForAdmin,
  countArchiveForAuthor,
...
export default async function ArchivePage({ searchParams }: PageProps) {
  ...
  // Both OWNER and ADMIN see all completed documents — OWNER doesn't author docs
  const [docs, total] = await Promise.all([
    listArchiveForAdmin(PAGE_SIZE, offset),
    countArchiveForAdmin(),
  ]);
    ...
    <div className="mx-auto flex max-w-5xl flex-col gap-6">
      ...
      {total === 0 ? (
        ...
        <>
          ...
          <Pagination page={page} pageSize={PAGE_SIZE} total={total} baseUrl="/archive" />
        </>
      )}
    </div>
  );
